...
class RevokedTokenValidatorTest {
    ...
    @Test
    @DisplayName("validate - Token Redis'te iptal edilmiş (revoked) ise failure dönmelidir")
    void validate_shouldReturnFailure_whenTokenIsRevoked() {
        when(tokenRevocationService.isTokenRevoked("jti-uuid-12345")).thenReturn(true);

        OAuth2TokenValidatorResult result = revokedTokenValidator.validate(sampleJwt);

        assertThat(result.hasErrors()).isTrue();
        assertThat(result.getErrors()).extracting("description").contains("The token is invalid or revoked");
        verify(tokenRevocationService).isTokenRevoked("jti-uuid-12345");
    }

    @Test
    @DisplayName("validate - Token iptal edilmemişse ve jti geçerli ise success dönmelidir")
    void validate_shouldReturnSuccess_whenTokenIsNotRevoked() {
        when(tokenRevocationService.isTokenRevoked("jti-uuid-12345")).thenReturn(false);
